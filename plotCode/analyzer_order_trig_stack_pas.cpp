...
for (int i=0;i<nfiles;i++){
	if (i==0) file_names[i] = data_name[set_type];
        file_names[i].Prepend("/afs/cern.ch/work/g/gimandor/private/Hmumu/restartFromFilippo/CMSSW_8_0_28/src/code/histoFileDir/"); 
...
/*            if (file_name_tag.find("HToMuMu")==std::string::npos) MCint+=histos_mc[hist]->Integral(0,histos_mc[hist]->GetNbinsX()+1); 
    ...
    ratioError[counter_ratio] = sqrt(DYError/DYint*DYError/DYint + dataError/dataInt*dataError/dataInt)*ratio[counter_ratio];
